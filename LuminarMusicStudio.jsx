...
export default function LuminarMusicStudio() {
  ...
  const generateSongGeneration = async () => {
    ...
    const input = aiMode === "beats" ? aiPrompt : `${aiLyrics}\n\nStyle: ${aiStyle}`;
    if (!input.trim()) { notify("Enter a prompt or lyrics", "warn"); return; }
    setSgLoading(true); setSgResult(null);
    try {
      const resp = await fetch("https://router.huggingface.co/hf-inference/models/facebook/musicgen-large", {
        method: "POST",
        headers: { Authorization: `Bearer ${hfKey}`, "Content-Type": "application/json" },
        body: JSON.stringify({ inputs: input, parameters: { max_new_tokens: 512 } })
      });
      if (!resp.ok) throw new Error("Status " + resp.status);
      const blob = await resp.blob();
      setSgResult(URL.createObjectURL(blob));
      notify("MusicGen ready!", "success");
    } catch (e) { notify("MusicGen failed: " + e.message, "error"); }
    setSgLoading(false);
  };
  ...
  const generateDiffRhythm = async () => {
    ...
    try {
      const resp = await fetch("https://router.huggingface.co/hf-inference/models/ASLP-lab/DiffRhythm-full", {
        method: "POST",
        headers: { Authorization: `Bearer ${hfKey}`, "Content-Type": "application/json" },
      ...
      if (!resp.ok) throw new Error("Status " + resp.status);
      const blob = await resp.blob();
      setDrResult(URL.createObjectURL(blob));
      notify("DiffRhythm ready!", "success");
    } catch (e) { notify("DiffRhythm failed: " + e.message, "error"); }
    setDrLoading(false);
  };
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 960, margin: "0 auto", padding: "20px 14px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <button onClick={() => setShowKeyInput(!showKeyInput)} style={{ ...btnStyle(hfKey ? "#6bcb77" : accent), padding: "8px 12px", display: "flex", alignItems: "center", gap: 6 }}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="7" cy="17" r="4"/><path d="M10.82 13.18L21 3"/><path d="M19 5l2 2"/><path d="M15 5l2 2"/></svg>
              <span>{hfKey ? "KEY SET" : "HF KEY"}</span>
            </button>
          </div>
        </div>
